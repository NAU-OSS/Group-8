...
//--------------Main program---------------\\
// This function specifies the two numbers for operator functions. we define c1,n1,d1, c2,n2, and d2 here and 
// we choose which operation(add or subtract) we want to use. we call our function in the Main function.
int main()
{
...
int numOfDigits(int numberToCount)
{
  ...
  while(numberToCount > 9)
  {
    ...
    amount++;             // for example, if our number is 150, first the 'amount' is equal to 1 then when we divide our number we get 15, and the 'amount" will be 
                          // 2 and then our number is 1.5 and the amount will be 3. so the number of digits will be equal to 3.
  }
